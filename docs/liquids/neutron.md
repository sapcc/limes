...
and if it is accepted by limes. Accepted resources are hardcoded in limes (see list below).
...
In addition we also support dynamically registered Neutron resources. These accepted based
on a list of prefixes. Currently this contains only `router_flavors_`. Note that this is a
SAP-specific add-on.
