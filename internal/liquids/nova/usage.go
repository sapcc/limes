...
// UnknownAZResourceUsageReports is a convenience constructor for PerAZ that puts all data in the "unknown" AZ.
// Adds zero-valued entries for all other AZs.
// Use this for data relating to AZ-aware resources where the AZ association is unknown.
func UnknownAZResourceUsageReports(data *liquid.AZResourceUsageReport, allAZs []liquid.AvailabilityZone) map[liquid.AvailabilityZone]*liquid.AZResourceUsageReport {
	usageReports := make(map[liquid.AvailabilityZone]*liquid.AZResourceUsageReport)
	for _, az := range allAZs {
		usageReports[az] = &liquid.AZResourceUsageReport{}
	}
	if data != nil {
		usageReports[liquid.AvailabilityZoneUnknown] = data
	}
...
func (l *Logic) ScanUsage(ctx context.Context, projectUUID string, req liquid.ServiceUsageRequest, serviceInfo liquid.ServiceInfo) (liquid.ServiceUsageReport, error) {
	...
	resources := map[liquid.ResourceName]*liquid.ResourceUsageReport{
		"cores": {
			...
			PerAZ: UnknownAZResourceUsageReports(&liquid.AZResourceUsageReport{Usage: absoluteLimits.TotalCoresUsed}, req.AllAZs),
		},
		"instances": {
			Quota: &absoluteLimits.MaxTotalInstances,
			PerAZ: UnknownAZResourceUsageReports(&liquid.AZResourceUsageReport{Usage: absoluteLimits.TotalInstancesUsed}, req.AllAZs),
		},
		"ram": {
			Quota: &absoluteLimits.MaxTotalRAMSize,
			PerAZ: UnknownAZResourceUsageReports(&liquid.AZResourceUsageReport{Usage: absoluteLimits.TotalRAMUsed}, req.AllAZs),
		},
		"server_groups": {
			Quota: &absoluteLimits.MaxServerGroups,
			PerAZ: UnknownAZResourceUsageReports(&liquid.AZResourceUsageReport{Usage: absoluteLimits.TotalServerGroupsUsed}, req.AllAZs),
		},
		"server_group_members": {
			Quota: &absoluteLimits.MaxServerGroupMembers,
			PerAZ: UnknownAZResourceUsageReports(&liquid.AZResourceUsageReport{Usage: totalServerGroupMembersUsed}, req.AllAZs),
		},
	}
	for flavorName, flavorLimits := range limitsData.Limits.AbsolutePerFlavor {
		...
		resources[resourceName] = &liquid.ResourceUsageReport{
			...
			PerAZ: UnknownAZResourceUsageReports(&liquid.AZResourceUsageReport{Usage: flavorLimits.TotalInstancesUsed}, req.AllAZs),
		}
	}
	for hwVersion, limits := range limitsData.Limits.AbsolutePerHWVersion {
		if l.hasPooledResource[hwVersion]["cores"] {
			resources[l.pooledResourceName(hwVersion, "cores")] = &liquid.ResourceUsageReport{
				...
				PerAZ: UnknownAZResourceUsageReports(&liquid.AZResourceUsageReport{Usage: limits.TotalCoresUsed}, req.AllAZs),
			}
		}
		if l.hasPooledResource[hwVersion]["instances"] {
			resources[l.pooledResourceName(hwVersion, "instances")] = &liquid.ResourceUsageReport{
				...
				PerAZ: UnknownAZResourceUsageReports(&liquid.AZResourceUsageReport{Usage: limits.TotalInstancesUsed}, req.AllAZs),
			}
		}
		if l.hasPooledResource[hwVersion]["ram"] {
			resources[l.pooledResourceName(hwVersion, "ram")] = &liquid.ResourceUsageReport{
				...
				PerAZ: UnknownAZResourceUsageReports(&liquid.AZResourceUsageReport{Usage: limits.TotalRAMUsed}, req.AllAZs),
			}
		}
	}
	...
	// calculate metrics
	var metrics map[liquid.MetricName][]liquid.Metric
	if l.HypervisorType != "" {
		countsByAZ := map[liquid.AvailabilityZone]uint64{liquid.AvailabilityZoneUnknown: 0}
		for _, subres := range allSubresources {
			var attrs SubresourceAttributes
			if err = json.Unmarshal(subres.Attributes, &attrs); err != nil { // TODO: Again unmarshalling
				return liquid.ServiceUsageReport{}, err
			}
			countsByAZ[attrs.AZ]++
		}

		countsByAZMetric := make([]liquid.Metric, len(countsByAZ))
		idx := 0
		for az, count := range countsByAZ {
			countsByAZMetric[idx] = liquid.Metric{
				Value:       float64(count),
				LabelValues: []string{l.HypervisorType, string(az)},
			}
			idx++
		}
		...
		metrics = map[liquid.MetricName][]liquid.Metric{
			"liquid_nova_instance_counts_by_hypervisor": {{
				Value:       float64(len(allSubresources)),
				LabelValues: []string{l.HypervisorType},
			}},
			"liquid_nova_instance_counts_bvy_hypervisor_and_az": countsByAZMetric,
		}
	}
