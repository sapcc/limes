...
func TestResourceRenaming(t *testing.T) {
	...
	baseBehaviors := []core.ResourceBehavior{
		...
		{FullResourceNameRx: "shared/things", CommitmentDurations: makeDurations(3 * time.Second)},
		{FullResourceNameRx: "unshared/capacity", CommitmentDurations: makeDurations(4 * time.Second)},
		{FullResourceNameRx: "unshared/things", CommitmentDurations: makeDurations(5 * time.Second)},
	}
