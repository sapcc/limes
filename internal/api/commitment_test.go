...
func TestCommitmentLifecycleWithImmediateConfirmation(t *testing.T) {
	...
	statsCache := datamodel.NewLazyStatsCache(s.DB, s.Cluster, loc)
	dbResult, err := statsCache.CanAcceptCommitmentChangeRequest(commitmentChangeRequest)
	...
	statsCache = datamodel.NewLazyStatsCache(s.DB, s.Cluster, loc) // stats changed, so recreate cache
	dbResult, err = statsCache.CanAcceptCommitmentChangeRequest(commitmentChangeRequest)
	assert.DeepEqual(t, "CanAcceptCommitmentChangeRequest", dbResult, true)
	if err != nil {
		t.Fatal(err)
