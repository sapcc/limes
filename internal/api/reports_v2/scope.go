...
// accessed with dbm. It returns the validated scope information or an error.
func (sc ScopeConfig) NewScope(r *http.Request, dbm *gorp.DbMap) (s Scope, err error) {
	...
	if sc.RequirePathProjectID || sc.AllowQueryProjectID {
		...
		case sc.RequirePathProjectID && pathProjectID == "":
			return s, errors.New("missing required path project_id")
