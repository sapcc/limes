...
func (p *v1Provider) CanConfirmNewProjectCommitment(w http.ResponseWriter, r *http.Request) {
	...
	}, loc.ServiceType, *serviceInfo, datamodel.NewLazyStatsCache(p.DB, p.Cluster, *loc))
	if respondwith.ObfuscatedErrorText(w, err) {
		return
	}
...
func (p *v1Provider) CreateProjectCommitment(w http.ResponseWriter, r *http.Request) {
	...
	commitmentChangeResponse, err := datamodel.DelegateChangeCommitments(r.Context(), p.Cluster, ccr, loc.ServiceType, *serviceInfo, datamodel.NewLazyStatsCache(p.DB, p.Cluster, *loc))
	if respondwith.ObfuscatedErrorText(w, err) {
		return
	}
...
func (p *v1Provider) MergeProjectCommitments(w http.ResponseWriter, r *http.Request) {
	...
	_, err = datamodel.DelegateChangeCommitments(r.Context(), p.Cluster, ccr, loc.ServiceType, serviceInfo, datamodel.NewLazyStatsCache(tx, p.Cluster, loc))
	if respondwith.ObfuscatedErrorText(w, err) {
		return
	}
...
func (p *v1Provider) DeleteProjectCommitment(w http.ResponseWriter, r *http.Request) {
	...
	_, err = datamodel.DelegateChangeCommitments(r.Context(), p.Cluster, ccr, loc.ServiceType, serviceInfo, datamodel.NewLazyStatsCache(p.DB, p.Cluster, loc))
	if respondwith.ObfuscatedErrorText(w, err) {
		return
	}
...
func (p *v1Provider) TransferCommitment(w http.ResponseWriter, r *http.Request) {
	...
	commitmentChangeResponse, err := datamodel.DelegateChangeCommitments(r.Context(), p.Cluster, ccr, loc.ServiceType, serviceInfo, datamodel.NewLazyStatsCache(tx, p.Cluster, loc))
	if respondwith.ObfuscatedErrorText(w, err) {
		return
	}
	if commitmentChangeRequestWasRejected(commitmentChangeResponse, w, true) {
...
func (p *v1Provider) ConvertCommitment(w http.ResponseWriter, r *http.Request) {
	...
	commitmentChangeResponse, err := datamodel.DelegateChangeCommitments(r.Context(), p.Cluster, ccr, sourceLoc.ServiceType, serviceInfo, datamodel.NewLazyStatsCache(tx, p.Cluster, targetLoc))
	if respondwith.ObfuscatedErrorText(w, err) {
		return
	}
