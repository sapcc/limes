...
func TestTranslateManilaSubcapacities(t *testing.T) {
	...
	subcapacitiesInLiquidFormat := []oldassert.JSONObject{
		{
			"name":     "pool1",
			"capacity": 520,
			"usage":    520,
			"attributes": oldassert.JSONObject{
	...
	subcapacitiesInLegacyFormat := []oldassert.JSONObject{
		{
			"pool_name":        "pool1",
			"az":               "az-one",
			"capacity_gib":     15360,
	...
	subcapacitiesInLegacyFormat := []oldassert.JSONObject{
		{
			...
			"ram":               oldassert.JSONObject{"value": 23, "unit": "MiB"},
			"disk":              oldassert.JSONObject{"value": 42, "unit": "GiB"},
			"cores":             5,
			"serial":            "98105291",
		},
		{
...
func testSubcapacityTranslation(t *testing.T, ruleID string, subcapacitiesInLiquidFormat, subcapacitiesInLegacyFormat []oldassert.JSONObject, srvInfo liquid.ServiceInfo) {
	s := test.NewSetup(t,
			...
			"cluster": oldassert.JSONObject{
				...
				"services": []oldassert.JSONObject{{
					...
					"resources": []oldassert.JSONObject{{
						...
						"domains_quota": 0,
						"usage":         0,
						"per_availability_zone": []oldassert.JSONObject{
							{"capacity": 0, "name": "az-one"},
							{"capacity": 0, "name": "az-two"},
						},
						"per_az": oldassert.JSONObject{
							"az-one": oldassert.JSONObject{"capacity": 0, "subcapacities": subcapacitiesInLegacyFormat},
							"az-two": oldassert.JSONObject{"capacity": 0},
						},
...
func TestTranslateCinderVolumeSubresources(t *testing.T) {
	subresourcesInLiquidFormat := []oldassert.JSONObject{
		{
	...
	subresourcesInLegacyFormat := []oldassert.JSONObject{
		{
			"id":                "6dfbbce3-078d-4c64-8f88-8145b8d44183",
			"name":              "volume1",
			"status":            "error",
			"size":              oldassert.JSONObject{"value": 21, "unit": "GiB"},
			"availability_zone": "az-one",
		},
		{
			"id":                "a33bae62-e14e-47a6-a019-5faf89c20dc7",
			"name":              "volume2",
			"status":            "available",
			"size":              oldassert.JSONObject{"value": 1, "unit": "GiB"},
			"availability_zone": "az-one",
		},
	}
...
func TestTranslateIronicSubresources(t *testing.T) {
	...
	subresourcesInLiquidFormat := []oldassert.JSONObject{
		...
		},
		{
			...
			"attributes": oldassert.JSONObject{
				"status":   "ACTIVE",
				"metadata": oldassert.JSONObject{"foo": "bar"},
				"tags":     []string{"foobar"},
	...
	subresourcesInLiquidFormat := []oldassert.JSONObject{
		{
			...
			"attributes": oldassert.JSONObject{
				"status": "ACTIVE",
				"metadata": oldassert.JSONObject{
					"image_buildnumber": "",
					"image_name":        "SAP-compliant-ubuntu-24-04",
				},
				"tags":              []string{},
				"availability_zone": "az-one",
				"flavor": oldassert.JSONObject{
					"name":          "g_c1_m2_v2",
					"vcpu":          1,
					"ram_mib":       2032,
					"disk_gib":      64,
	...
	subresourcesInLegacyFormat := []oldassert.JSONObject{
		{
			...
			"flavor":            "g_c1_m2_v2",
			"vcpu":              1,
			"ram": oldassert.JSONObject{
				"value": 2032,
				"unit":  "MiB",
			},
			"disk": oldassert.JSONObject{
				"value": 64,
				"unit":  "GiB",
			},
			"video_ram": oldassert.JSONObject{
				"value": 16,
				"unit":  "MiB",
			},
			"os_type": "image-deleted",
...
func testSubresourceTranslation(t *testing.T, ruleID string, subresourcesInLiquidFormat, subresourcesInLegacyFormat []oldassert.JSONObject, srvInfo liquid.ServiceInfo) {
	s := test.NewSetup(t,
		test.WithConfig(testTranslationConfigJSON),
		test.WithPersistedServiceInfo("first", srvInfo),
		test.WithInitialDiscovery,
						...
						"resources": []oldassert.JSONObject{
							{
								...
								"usage":        0,
								"per_az": oldassert.JSONObject{
									"az-one": oldassert.JSONObject{"usage": 0, "subresources": subresourcesInLegacyFormat},
									"az-two": oldassert.JSONObject{"usage": 0},
								},
								"quota_distribution_model": "autogrow",
