...
func (p *v2Provider) handleGetResourcesCluster(r *http.Request, token *gopherpolicy.Token) (none resourcesv2.InfoReport, err error) {
	httpapi.IdentifyEndpoint(r, "/resources/v2/cluster")
	err = token.Enforce("v2:cluster:report")
	if err != nil {
		return none, err
	}
	_, err = reports_v2.ScopeConfig{}.NewScope(r, p.DB)
	if err != nil {
		return none, err
	}
	return none, nil
