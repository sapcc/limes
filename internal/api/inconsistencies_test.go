...
func TestInconsistencyReport(t *testing.T) {
	...
	s.MustDBExec(`UPDATE project_az_resources SET usage = 20 WHERE az_resource_id = $1`,
	...
	s.MustDBExec(`UPDATE project_az_resources SET backend_quota = 10 WHERE project_id = $1 AND az_resource_id IN (SELECT id FROM az_resources WHERE resource_id = $2 AND az = $3)`,
		s.GetProjectID("dresden"),
		s.GetResourceID("shared", "capacity"),
		liquid.AvailabilityZoneTotal,
	)
	s.MustDBExec(`UPDATE project_az_resources SET quota = 14, backend_quota = 14 WHERE project_id = $1 AND az_resource_id IN (SELECT id FROM az_resources WHERE resource_id = $2 AND az = $3)`,
