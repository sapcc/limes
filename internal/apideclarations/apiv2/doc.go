...
// A project-scoped token can only access this path, if its project_uuid is set in the query parameter or when public=true.
// A domain-scoped token can only access this path, if a project_uuid from his domain or its domain_uuid is set in the query parameter or when public=true.
// A cloud-admin token can receive data for all filters.
//   - Takes query parameters as defined in [common.CommitmentListOpts].
