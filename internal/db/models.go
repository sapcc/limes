...
type ClusterCapacitor struct {
	...
	ScrapedAt          Option[time.Time] `db:"scraped_at"` // pointer type to allow for NULL value
...
type ProjectService struct {
	...
	SerializedMetrics       string            `db:"serialized_metrics"`
	QuotaDesyncedAt         Option[time.Time] `db:"quota_desynced_at"`
...
type ProjectCommitment struct {
	...
	PredecessorID Option[ProjectCommitmentID] `db:"predecessor_id"`
