...
var sqlMigrations = map[string]string{
	...
	`),
	"070_remove_project_level_values.down.sql": `
	...
	`,
	"070_remove_project_level_values.up.sql": `
		...
		INSERT INTO az_resources (resource_id, az, raw_capacity, usage, subcapacities, last_nonzero_raw_capacity, path)
			SELECT azr.resource_id, 'total' as az, SUM(raw_capacity), SUM(usage), '' AS subcapacities, NULL AS last_nonzero_raw_capacity, REPLACE(azr.path, azr.az, 'total') AS path
            FROM az_resources azr
