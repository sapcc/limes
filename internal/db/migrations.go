...
var sqlMigrations = map[string]string{
  ...
  `,
	...
	`,
	"058_project_level_v2_artifacts.down.sql": `
	...
	`,
	"058_project_level_v2_artifacts.up.sql": `
		...
		CREATE TABLE project_commitments_v2 (
		    id								BIGSERIAL	NOT NULL PRIMARY KEY,
		    project_id						BIGINT		NOT NULL REFERENCES projects ON DELETE CASCADE,
		    az_resource_id					BIGINT		NOT NULL REFERENCES cluster_az_resources ON DELETE RESTRICT,
		    amount							BIGINT		NOT NULL,
		    duration						TEXT		NOT NULL,
			created_at						TIMESTAMP	NOT NULL,
			creator_uuid					TEXT		NOT NULL,
		...
		INSERT INTO project_rates_v2 ( project_id, rate_id, rate_limit, window_ns, usage_as_bigint )
			...
			JOIN projects p ON ps.project_id = p.id
			JOIN cluster_services cs ON ps.type = cs.type
			JOIN cluster_rates cr ON cs.id = cr.service_id AND pr.name = cr.name;
