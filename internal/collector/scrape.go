...
func (c *Collector) writeDummyResources(dbDomain db.Domain, dbProject db.Project, srv db.Service) error {
	...
	_, err = tx.Select(&azResources, `SELECT * FROM az_resources WHERE resource_id IN (SELECT id FROM resources WHERE service_id = $1) AND az IN ($2, $3)`, srv.ID, liquid.AvailabilityZoneAny, liquid.AvailabilityZoneTotal)
	if err != nil {
		return err
	}
