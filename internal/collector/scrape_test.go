...
func Test_ScrapeSuccess(t *testing.T) {
	...
	srvInfo := must.ReturnT(s.Cluster.AllServiceInfos())(t)["unittest"]
	for resName, resInfo := range srvInfo.Resources {
		must.SucceedT(t, datamodel.ApplyComputedProjectQuota("unittest", resName, resInfo, s.Cluster, s.Clock.Now()))
	}
