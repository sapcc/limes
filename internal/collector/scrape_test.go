...
func Test_ScrapeSuccess(t *testing.T) {
	...
	scrapedAt1 = s.Clock.Now().Add(-5 * time.Second)
	scrapedAt2 = s.Clock.Now()
	// TODO: I do not see where the changes to project_az_resources are coming from.
	// Are they correct?
	tr.DBChanges().AssertEqualf(`
		UPDATE project_az_resources SET historical_usage = '{"t":[10875],"v":[20]}' WHERE id = 2 AND resource_id = 1 AND az = 'az-one';
		UPDATE project_az_resources SET historical_usage = '{"t":[10880],"v":[20]}' WHERE id = 8 AND resource_id = 3 AND az = 'az-one';
