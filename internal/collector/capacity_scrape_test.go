...
const commitmentConfigWithoutOvercommit = `
			availability_zones: [ az-one, az-two ]
			discovery:
				method: static
				static_config:
...
func Test_ScanCapacityWithCommitmentTakeover(t *testing.T) {
	s, add := commonScanCapacityWithCommitmentsSetup(t, commitmentConfigWithoutOvercommit)
	job := s.Collector.CapacityScrapeJob(s.Registry)
...
func TestScanCapacityWithMailNotification(t *testing.T) {
	s, _ := commonScanCapacityWithCommitmentsSetup(t, commitmentConfigWithoutOvercommit+`
			mail_notifications:
				templates:
