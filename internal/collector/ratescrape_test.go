		...
		services:
			- service_type: unittest
				...
				params:
					area: testing
					test_mode: true
					liquid_service_type: someServiceName
	`
)
...
func commonRateScrapeTestSetup(t *testing.T, s *test.Setup) (job jobloop.Job, withLabel jobloop.Option) {
	// s.Cluster.QuotaPlugins["unittest"].(*plugins.LiquidQuotaPlugin).LiquidServiceInfo.Version = 1
...
func Test_RateScrapeSuccess(t *testing.T) {
	...
	tr.DBChanges().AssertEqualf(`
		...
		UPDATE project_services SET rates_scraped_at = %[1]d, rates_scrape_duration_secs = 5, rates_scrape_state = '{"firstrate":1024,"secondrate":2048}', rates_checked_at = %[1]d, rates_next_scrape_at = %[2]d WHERE id = 1 AND project_id = 1 AND type = 'unittest';
		UPDATE project_services SET rates_scraped_at = %[3]d, rates_scrape_duration_secs = 5, rates_scrape_state = '{"firstrate":1024,"secondrate":2048}', rates_checked_at = %[3]d, rates_next_scrape_at = %[4]d WHERE id = 2 AND project_id = 2 AND type = 'unittest';
	`,
...
func Test_RateScrapeFailure(t *testing.T) {
	...
	expectedErrorRx := regexp.MustCompile(`^during rate scrape of project germany/(berlin|dresden): failed to get usage report$`)
	...
	client.FailUsageReport = true
	mustFailLikeT(t, job.ProcessOne(s.Ctx, withLabel), expectedErrorRx)

	checkedAt := s.Clock.Now()
	tr.DBChanges().AssertEqualf(`
		UPDATE project_services SET rates_checked_at = %[1]d, rates_scrape_error_message = 'failed to get usage report', rates_next_scrape_at = %[2]d WHERE id = 1 AND project_id = 1 AND type = 'unittest';
	`,
