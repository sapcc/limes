...
func Test_RateScrapeSuccess(t *testing.T) {
	...
	tr0.AssertEqualf(`
		...
		INSERT INTO project_services (id, project_id, type) VALUES (1, 1, 'unittest');
		INSERT INTO project_services (id, project_id, type) VALUES (2, 2, 'unittest');
	...
	tr.DBChanges().AssertEqualf(`
		...
		UPDATE project_services SET rates_scraped_at = 1, rates_scrape_duration_secs = 1, rates_scrape_state = '{"firstrate":0,"secondrate":0}', rates_checked_at = 1 WHERE id = 1 AND project_id = 1 AND type = 'unittest';
		UPDATE project_services SET rates_scraped_at = 3, rates_scrape_duration_secs = 1, rates_scrape_state = '{"firstrate":0,"secondrate":0}', rates_checked_at = 3 WHERE id = 2 AND project_id = 2 AND type = 'unittest';
	`)
	...
	tr.DBChanges().AssertEqualf(`
		...
		UPDATE project_services SET rates_scraped_at = 7, rates_scrape_state = '{"firstrate":5120,"secondrate":1024}', rates_checked_at = 7 WHERE id = 1 AND project_id = 1 AND type = 'unittest';
		UPDATE project_services SET rates_scraped_at = 9, rates_scrape_state = '{"firstrate":1024,"secondrate":1024}', rates_checked_at = 9 WHERE id = 2 AND project_id = 2 AND type = 'unittest';
	`)
...
func Test_RateScrapeFailure(t *testing.T) {
	...
	c.ScrapeRates()
	tr.DBChanges().AssertEqualf(`
		UPDATE project_services SET rates_scrape_duration_secs = 1, rates_checked_at = 1, rates_scrape_error_message = 'ScrapeRates failed as requested' WHERE id = 1 AND project_id = 1 AND type = 'unittest';
	`)
}
...
func Test_ScrapeRatesButNoRates(t *testing.T) {
	...
	tr0.AssertEqualf(`
		...
		INSERT INTO domains (id, name, uuid) VALUES (1, 'germany', 'uuid-for-germany');
		INSERT INTO project_services (id, project_id, type, rates_scraped_at, rates_scrape_duration_secs, rates_checked_at) VALUES (1, 1, 'noop', 1, 1, 1);
		INSERT INTO projects (id, domain_id, name, uuid, parent_uuid, has_bursting) VALUES (1, 1, 'berlin', 'uuid-for-berlin', 'uuid-for-germany', FALSE);
	`)
}
