...
func ConfirmPendingCommitments(ctx context.Context, loc core.AZResourceLocation, unit limes.Unit, cluster *core.Cluster, dbi db.Interface, now time.Time, generateProjectCommitmentUUID func() liquid.CommitmentUUID, generateTransferToken func() string, auditContext audit.Context) (auditEvents []audittools.Event, err error) {
	...
	affectedDomainsByID, err := db.BuildIndexOfDBResult(dbi, func(d db.Domain) db.DomainID { return d.ID }, `SELECT * FROM domains WHERE id IN (SELECT id FROM projects WHERE id = ANY($1))`, pq.Array(slices.Collect(maps.Keys(affectedProjectIDs))))
	if err != nil {
	...
	for _, c := range confirmableCommitments {
		...
		capacityAccepted, err := checkCommitmentAcceptance(ctx, cluster, dbi, loc, serviceInfo, project, domain, c, stats.ProjectStats[c.ProjectID].Committed)
		if err != nil {
...
func checkCommitmentAcceptance(ctx context.Context, cluster *core.Cluster, dbi db.Interface, loc core.AZResourceLocation, serviceInfo liquid.ServiceInfo, project db.Project, domain db.Domain, commitment db.ProjectCommitment, totalConfirmedBefore uint64) (bool, error) {
		...
		ByProject: map[liquid.ProjectUUID]liquid.ProjectCommitmentChangeset{
			project.UUID: {
				...
				ByResource: map[liquid.ResourceName]liquid.ResourceCommitmentChangeset{
					loc.ResourceName: {
						TotalConfirmedBefore: totalConfirmedBefore,
						TotalConfirmedAfter:  totalConfirmedBefore + commitment.Amount,
	...
	commitmentChangeResponse, err := DelegateChangeCommitments(cluster, ctx, commitmentChangeRequest, loc.ServiceType, serviceInfo, dbi)
	if err != nil {
		return false, err
	}

	// as the totalConfirmed will increase, we don't need to check for
	return commitmentChangeResponse.RejectionReason == "", nil
}
