...
var (
	...
	getTransferCommitmentsQuery = sqlext.SimplifyWhitespace(db.ExpandEnumPlaceholders(`
		...
		JOIN az_resources cazr ON cazr.resource_id = cr.id
		JOIN project_commitments pc ON pc.az_resource_id = cazr.id
		WHERE cs.type = $1 AND cr.name = $2 AND cazr.az = $3 AND pc.transfer_status = {{limesresources.CommitmentTransferStatusPublic}}
...
func ConfirmPendingCommitments(loc core.AZResourceLocation, cluster *core.Cluster, dbi db.Interface, now time.Time, generateProjectCommitmentUUID func() liquid.CommitmentUUID, generateTransferToken func() string) ([]db.MailNotification, error) {
	...
	for _, c := range confirmableCommitments {
		...
		for idx, t := range transferableCommitments {
			...
			if t.Amount > leftoverConsumptionAmount {
				...
				leftoverCommitment, err := util.BuildSplitCommitment(t, t.Amount-leftoverConsumptionAmount, now, generateProjectCommitmentUUID)
				if err != nil {
					return nil, err
				}
	...
	// now update the consumed commitments
	/*for _, t := range transferableCommitments {
		consumption, exists := consumedAmounts[t.ID]
		// as we consume in the order of the array, after the first unconsumed commitment we can break
		if !exists {
			break
		}
		// in any case, we supersede the commitment. The difference is only whether we have a leftover or not.
		if t.Amount > consumption.Amount {
			// write a superseding commitment (in transfer) for the leftover
			leftoverCommitment, err := util.BuildSplitCommitment(t, t.Amount-consumption.Amount, now, generateProjectCommitmentUUID)
			// this commitment needs a new transfer token, because they must be unique
			// but it needs the old transferStartedAt so that it receives the same priority
			leftoverCommitment.TransferStatus = limesresources.CommitmentTransferStatusPublic
			leftoverCommitment.TransferToken = Some(generateTransferToken())
			leftoverCommitment.TransferStartedAt = t.TransferStartedAt
			if err != nil {
		...
		supersedeContext := db.CommitmentWorkflowContext{
			Reason:                 db.CommitmentReasonTransfer,
			RelatedCommitmentIDs:   consumption.RelatedIDs,
			RelatedCommitmentUUIDs: consumption.RelatedUUIDs,
		}
		buf, err := json.Marshal(supersedeContext)
		if err != nil {
			return nil, err
