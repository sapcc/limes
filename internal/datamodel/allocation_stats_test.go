...
func TestCanAcceptCommitmentChanges(t *testing.T) {
	...
	ccr := core.CommitmentCreationRule{
		UntilPercent: p2f64(100),
	}
	additions := map[db.ProjectResourceID]uint64{2: 300}
	result := stats.CanAcceptCommitmentChanges(additions, nil, ccr)
	...
	ccrRestrictive := core.CommitmentCreationRule{
		UntilPercent: p2f64(50),
	}
	additions = map[db.ProjectResourceID]uint64{2: 300}
	result = stats.CanAcceptCommitmentChanges(additions, nil, ccrRestrictive)
	...
	result = stats.CanAcceptCommitmentChanges(nil, subtractions, ccr)
	assert.DeepEqual(t, "CanAcceptCommitmentChanges", result, true)
}
