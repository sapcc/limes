...
func DelegateChangeCommitments(ctx context.Context, cluster *core.Cluster, req liquid.CommitmentChangeRequest, serviceType db.ServiceType, serviceInfo liquid.ServiceInfo, statsCache *LazyStatsCache) (result liquid.CommitmentChangeResponse, err error) {
	...
	if len(localCommitmentChanges.ByProject) != 0 {
		canAcceptLocally, err := statsCache.CanAcceptCommitmentChangeRequest(localCommitmentChanges)
		if err != nil {
