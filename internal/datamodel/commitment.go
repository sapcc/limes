...
func ConvertCommitmentToDisplayForm(c db.ProjectCommitment, az limes.AvailabilityZone, apiIdentity core.ResourceRef, canBeDeleted bool, unit limes.Unit) limesresources.Commitment {
	return limesresources.Commitment{
		...
		TransferStartedAt: options.Map(c.TransferStartedAt, util.IntoUnixEncodedTime).AsPointer(),
		Status:            c.Status,
...
func DelegateChangeCommitments(ctx context.Context, cluster *core.Cluster, req liquid.CommitmentChangeRequest, sis core.ServiceInfoSnapshot, serviceType db.ServiceType, dbi db.Interface) (result liquid.CommitmentChangeResponse, err error) {
	...
	if len(localCommitmentChanges.ByProject) != 0 {
		canAcceptLocally, err := CanAcceptCommitmentChangeRequest(ctx, localCommitmentChanges, sis, service.Type, cluster, dbi)
		if err != nil {
...
// ProjectMetadata is always filled for Limes-internal use. Before delegating it to the liquid,
// it will be omitted if not requested.
