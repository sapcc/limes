...
func NewTransferableCommitmentCache(dbi db.Interface, loc util.AZResourceLocation, now time.Time, generateProjectCommitmentUUID func() liquid.CommitmentUUID, generateTransferToken func() string) (t TransferableCommitmentCache, err error) {
...
func (t *TransferableCommitmentCache) GenerateAuditEventsAndMails(apiIdentity util.ResourceRef, unit limes.Unit, auditContext audit.Context, mailTemplate Option[core.MailTemplate]) (auditEvents []audittools.Event, err error) {
