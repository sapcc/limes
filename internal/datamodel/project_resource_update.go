...
func (u ProjectResourceUpdate) Run(dbi db.Interface, serviceInfos map[db.ServiceType]liquid.ServiceInfo, now time.Time, domain db.Domain, project db.Project, srv db.ServiceRef[db.ProjectServiceID]) ([]db.ProjectResource, error) {
	if u.LogError == nil {
	...
	for resName, resInfo := range core.ResourcesForService(serviceInfos, srv.Type) {
